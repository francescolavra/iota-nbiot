...
#if __linux
int SerialBase::linuxUartOpen(const char *port)
{
	...
	if (cfgetispeed(&termios) != linuxBaud) {
		if (cfsetispeed(&termios, linuxBaud)) {
			perror("Cannot set UART baud rate");
			goto error;
		}
	...
	if (cfgetospeed(&termios) != linuxBaud) {
		if (cfsetospeed(&termios, linuxBaud)) {
			goto error;
		}
		if (tcsetattr(fd, TCSAFLUSH, &termios)) {
			goto error;
